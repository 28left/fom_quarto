...
- **(iii)** $(\forall \xi < \lambda \; \varphi(\xi)) \to \varphi(\lambda))$ for all limit $\lambda$,
...
::: {#thm-ordinal-recursion}
## Recursion on ordinals
\
If $G : \V \longrightarrow  \V$ is a function, then there exists a unique function $F: \Ord \longrightarrow \V$  such that for all $\alpha \in \Ord$,

$$
F(\alpha) = G(F\Rest{\alpha})
$$
:::
...
- Call $h$ *tame* if 
\begin{equation*}
\exists \alpha \, (h: \alpha \to \V  \wedge \forall \xi \in \alpha \; h(\xi) = G(h \Rest{\xi}))
\end{equation*}
...
If $D = \Op{Dom}(F) \neq \Ord$, then we would have $D = \alpha$ for some ordinal ${}\alpha$. In particular $D$ is a set therefore $F = f$ is a set, for some tame $f$. This $f$ could be extended to a tame $h = f \cup \{(\alpha,G(f \Rest{\alpha}))\}$, contradiction.
:::

Note that we defined $F$ **explicitly** as a *union* of all partial solutions to the recursion equation. (Of course, since $F$ is a proper class, one has the translate the informal definition of $F$ above into a formal expression.)


As with induction, we have the following variant of the recursion principle.

::: {#prp-ordinal-recursion-ii}
