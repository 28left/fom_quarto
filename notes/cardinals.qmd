...
"Proof by picture"
![Proof of the Cantor-Schröder-Bernstein Theorem](./images/schroeder-bernstein.png){width=80% fig-align="center"}
::: -->
...
::: {.proof collapse="true"}
By the well-ordering principle $\WO$, there exist ordinals $\alpha, \beta$ such that $a \sim \alpha$ and $b \sim \beta$. Since any two ordinals are comparable by $<$, $\alpha \subseteq \beta$ or $\beta \subseteq \alpha$, which easily yields the theorem.
:::
...
The mapping $\alpha \mapsto \aleph_\alpha$ is stricly increasing. It follows that $\kappa \leq \aleph_\kappa$ and hence $\kappa < \aleph_{\kappa+1}$. Choose ${}\alpha$ minimal such that $\kappa < \aleph_\alpha$. A simple case distinction yields that ${}\alpha$ must be a successor ordinal, $\alpha = \beta+1$. It then follows from $\aleph_{\beta+1} = \aleph_\beta^+$ that $\kappa = \aleph_\beta$.
:::
...
$$
Note that $f(\alpha) := F(\alpha\times \alpha)$ is a strictly increasing map on ordinals, and hence $F(\alpha) \geq \alpha$.
Further note that 
$$
...
Note that since for any $\beta$, $F(\beta,\beta) \geq \beta$, we must have $F(\aleph_\alpha, \aleph_\alpha) > \aleph_\alpha$. 
...
::: {.callout-important}
## Continuum Hypothesis ($\CH$ – cardinal version) 
